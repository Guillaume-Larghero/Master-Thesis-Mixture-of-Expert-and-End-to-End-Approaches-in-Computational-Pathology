...
class TransformerModule(nn.Module):
    ...
    def forward(
        ...
        mm = torch.matmul(a, h)  # (N_CLASSES, 192)
...
class HIPTMILWrap(nn.Module):
    """
    Wrapper class, since the model expects a batch size of 1
    but we want to use it as drop-in replacement for other models
    in our existing pipeline.
    """
    def __init__(self, **kwargs):
        super().__init__()
        self.model = HIPTMIL(**kwargs)

    def forward(self, x):
        B, _, _ = x.shape
        outputs = []
        for i in range(B):
            outputs.append(self.model(x[i].unsqueeze(0)))
        return torch.cat(outputs, dim=0)


if __name__ == "__main__":
    model = HIPTMIL(in_features=100, out_features=1, metadata_cols=0)
    x = torch.rand(1, 10, 100)
    mask = torch.tensor([[[False]*10]]).unsqueeze(-1)
    logits = model(x, mask)
