...
class MaxPool(torch.nn.Module):
    """MaxPool model.
    A simple model that takes the maximum of tile features of a slide as input
    (max-pooled over the tiles dimension).
    ...
    hidden: Optional[List[int]] = None
        Number of units for layers of the MLP. If `None`, a linear layer is
        used to compute the prediction from the max-pooled tile features.
        If e.g. `[128, 64]`, the prediction is computed with a
        MLP features_dim -> 128 -> 64 -> 1.
    dropout: Optional[List[float]] = None,
        Dropout that is used for each layer of the MLP. If `None`, no dropout
    ...
    def _max(self, x: torch.Tensor, mask: Optional[torch.BoolTensor] = None):
        if mask is not None:
            # Masked elements are set to a very low value, so they don't affect the max operation
            max_x = torch.max(x.masked_fill(mask, -float('inf')), dim=1)[0]
        else:
    ...
    def forward(
        ...
        """
        features = x[..., self.metadata_cols:]
        max_feats = self._max(features, mask)
        return self.mlp(max_feats)