...
from models.owkin.utils.extreme_layer import ExtremeLayer
...
class DSMIL(nn.Module):
    ...
    def __init__(
        ...
        self.extreme_layer = ExtremeLayer(n_top=1, n_bottom=0, return_indices=True)
        ...
        mlp_in_features = d_tiles_values if self.passing_values else in_features
    ...
    def forward(
        ...
        n_tiles_logits, ind_n_tiles = self.extreme_layer(x=tiles_scores, mask=mask)
...
class DSMILWrap(nn.Module):
    """
    Wrapper for DSMIL model.
    In our general pipeline we might not have masks.
    """

    def __init__(self, **kwargs):
        super().__init__()
        self.model = DSMIL(**kwargs)

    def forward(self, x, mask: Optional[torch.Tensor] = None):
        B, N, _ = x.shape
        if mask is None:  # create fake mask
            mask = (
                torch.tensor([[False] * N for _ in range(B)]).unsqueeze(-1).to(x.device)
            )
        return self.model(x, mask).squeeze()


if __name__ == "__main__":
    # python -m models.dsmil
    x = torch.rand(2, 10, 103)

    # indicate valid tiles (None of the tiles are padded in this example)
    mask = torch.tensor([[False] * 10] * 2).unsqueeze(-1)

    dsmil = DSMIL(
        in_features=100,
        d_tiles_values=32,
        d_tiles_queries=32,
        tiles_scores_mlp_hidden=[50],
        tiles_values_mlp_hidden=[50],
        tiles_queries_mlp_hidden=[50],
        mlp_hidden=[50],
        mlp_dropout=[0.2],
    )
