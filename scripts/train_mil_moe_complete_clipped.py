...
metricfile = "/n/data2/hms/dbmi/kyu/lab/gul075/MOE_NEW_Metrics_clipped.csv"
...
def train_k_fold(cfg: DictConfig):
    ...
    for fold in range(cfg.data.folds):
        ...
        for epoch in range(cfg.train.epochs):
            ...
            if ((fisrt_save==False) and (metrics["balanced_acc"] >= 0.87) and (metrics["auc_pr"] >= 0.8) and (metrics["weighted_f1"] >= 0.87) and (metrics["roc_auc"]>=0.85)):
        ...
        model.load_state_dict(torch.load(f"{cfg.checkpoints.save_dir}/clipped/{cfg.logger.experiment_id}_CLIPPEDSAVE_{fold}.pth"))
        ...
        with torch.no_grad():
            ...
            update_metric_file(model_name = f"{cfg.logger.experiment_id}_{fold}_CLIPPED", cohort = cfg.data.cohort , metrics = test_metrics)
            print("Done!")
        ...
        for cohort in cohorts:
            ...
            with torch.no_grad():
                ...
                update_metric_file(model_name = f"{cfg.logger.experiment_id}_{fold}_CLIPPED", cohort = cohort , metrics = inf_metrics)
                print("Done!")
        fisrt_save =False
...
def save_model_clip(cfg, model, fold):
    ...
    model_pth = f"{cfg.logger.experiment_id}_CLIPPEDSAVE_{fold}.pth"
