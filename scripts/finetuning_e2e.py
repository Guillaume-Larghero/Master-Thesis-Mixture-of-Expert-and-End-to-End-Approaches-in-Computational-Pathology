...
metricfile = "/n/data2/hms/dbmi/kyu/lab/gul075/SC_Metrics_clipped_FT.csv"
...
def save_model_clip(cfg, model, fold):
    ...
    model_pth = f"{cfg.logger.experiment_id}_SC_CLIPPEDSAVE_{fold}_FT.pth"
...
def get_datasets_for_fold(cfg,cohort, fold):
    # Load the label file
    labels_df = pd.read_excel(f"/n/data2/hms/dbmi/kyu/lab/gul075/{cohort}_Labels_SC_{cfg.data.label_column}.xlsx")
...
def main(cfg: DictConfig):
    ...
    for fold in range(5):
        ...
        for cohort in cohorts:
            ...
            cfg.logger.experiment_id = f"{modelname}_FINETUNE_ON_{cohort}"
            ...
            train, val, test = get_datasets_for_fold(cfg,cohort,fold)
            ...
            if num_pos > 0:
                ...
                pos_weight = torch.FloatTensor([1.0]).to(cfg.train.device)
            ...
            model.load_state_dict(torch.load(f"{cfg.checkpoints.save_dir}/clipped/{cfg.logger.experiment_id}_SC_CLIPPEDSAVE_{fold}_FT.pth"))
            ...
            with torch.no_grad():
                ...
                update_metric_file(model_name=f"{cfg.logger.experiment_id}_{fold}_SC_CLIPPED_FT", cohort=cfg.data.cohort, metrics=test_metrics)
                print("Done!")
